   	...
   	$.getJSON('https://twitter.com/status/user_timeline/zhihaojia.json?count=12&exclude_replies=true&callback=?', function(data) {
        ...
        var items = [];
   	    $.each(data, function(index, item) {
            ...
            items.push('<li>' + item.text.linkify() + ' <span class="faded">(<a href="https://twitter.com/ZhihaoJia/statuses/' + item.id_str  + '" target="_blank">' + relative_time(item.created_at) + ')</a></span></li>');
   	    });

        // use html string to check whether any items were retrieved
        if (items.length === 0) {
            ...
            $.getJSON('https://api.twitter.com/account/rate_limit_status.json?callback=?', function(data) {
                if (data.remaining_hits <= 0) {
                    $('<p/>', {
                        'class': 'msg',
                        html: 'Maximum number of requests reached.'
